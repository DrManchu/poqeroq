...
class Config:
    ...
    # Таймауты (в секундах)
    JOIN_TIMEOUT: int = 60       # Время на присоединение к столу (не используется)
    TURN_TIMEOUT: int = 90       # Время на принятие решения во время игры
    NEXT_HAND_DELAY: int = 10    # Пауза между раздачами (не используется)
    ...
    # Список администраторов (usernames без @)
    ADMIN_USERNAMES: list = None  # Будет загружен из переменной окружения

    def __post_init__(self):
        """Инициализация после создания"""
        # Загружаем список админов из переменной окружения
        admin_str = os.getenv('POKER_ADMINS', '')
        if admin_str:
            self.ADMIN_USERNAMES = [u.strip().lower() for u in admin_str.split(',') if u.strip()]
        else:
