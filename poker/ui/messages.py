...
def format_showdown_message(
    ...
    # Победители (фильтруем возвраты для правильного отображения)
    real_winners = [(w, amt, combo) for w, amt, combo in winners if combo != "Возврат ставки"]
    refunds = [(w, amt, combo) for w, amt, combo in winners if combo == "Возврат ставки"]

    if len(real_winners) == 1 and not refunds:
        # Один победитель, нет возвратов
        winner, amount, combo = real_winners[0]
    ...
    elif len(real_winners) == 1 and refunds:
        # Один победитель + возврат другому игроку
        winner, amount, combo = real_winners[0]
        winner_name = get_display_name(winner.first_name, winner.username)
        lines.append("")
        lines.append(f"🎉 <b>Победитель:</b> {winner_name}")
        ...
        lines.append(f"🃏 {combo}")
        lines.append("")
        for refund_player, refund_amt, _ in refunds:
            refund_name = get_display_name(refund_player.first_name, refund_player.username)
            lines.append(f"↩️ Возврат: {refund_name} получил {format_chips(refund_amt)} 🪙")
    elif len(real_winners) > 1:
        # Реальный сплит
        lines.append("")
        lines.append(f"🎉 <b>Победители (сплит):</b>")
        for winner, amount, combo in real_winners:
            winner_name = get_display_name(winner.first_name, winner.username)
        ...
        # Только возвраты (странная ситуация, но обработаем)
        lines.append("")
        lines.append("↩️ <b>Возврат ставок</b>")
        for refund_player, refund_amt, _ in refunds:
            refund_name = get_display_name(refund_player.first_name, refund_player.username)
            lines.append(f"  • {refund_name}: {format_chips(refund_amt)} 🪙")
