...
async def cb_join(callback: CallbackQuery) -> None:
    ...
    # БЛОКИРОВКА: Нельзя присоединяться во время активной игры
    if game.status not in [GameStatus.WAITING, GameStatus.SHOWDOWN, GameStatus.FINISHED]:
        await callback.answer("⚠️ Идёт игра! Дождитесь окончания раздачи.", show_alert=True)
        return
...
async def cb_leave(callback: CallbackQuery) -> None:
    ...
    # БЛОКИРОВКА: Нельзя выходить во время активной игры (только фолд)
    if game.status not in [GameStatus.WAITING, GameStatus.SHOWDOWN, GameStatus.FINISHED]:
        await callback.answer("⚠️ Идёт игра! Чтобы выйти, используйте кнопку 'Fold' (сброс).", show_alert=True)
        return
    
    # 1. Если игра завершена (Showdown/FINISHED) - выходим без пересчета банка
    if game.status in [GameStatus.SHOWDOWN, GameStatus.FINISHED]:
        winnings_text = ""
        current_win = getattr(player, 'current_win', 0)

        if current_win > 0:
            winnings_text = f". 💰 Вы забрали: {format_chips(current_win)} 🪙"
            # Баланс уже обновлён в update_game_message, просто сохраняем
            await db.set_balance(user_id, player.balance)

        # КРИТИЧЕСКИ ВАЖНО: Полностью удаляем игрока из списка, а не просто помечаем неактивным
...
async def update_game_message(bot: Bot, game: PokerGame, time_remaining: int = None) -> None:
    ...
    if game.status in [GameStatus.SHOWDOWN, GameStatus.FINISHED] and game.pot_distributed:
        # Банк уже распределён - просто показываем результаты из кэша
        ...
        # Первый раз в SHOWDOWN - вычисляем и распределяем банк
        winners = game.determine_winners()
        msg_text = format_showdown_message(game, game.players, winners)
        keyboard = get_showdown_keyboard(game.game_id)

        # КРИТИЧЕСКИ ВАЖНО: Применяем выигрыши только один раз
        if not game.pot_distributed:
            game.pot_distributed = True
            logger.info(f"Игра {game.game_id}: распределение банка после showdown (первый раз)")

            # Начисляем балансы победителям
            winners_ids = set()
            for winner, amount, desc in winners:
                winner.balance += amount
                winners_ids.add(winner.user_id)
                logger.info(f"Игра {game.game_id}: {winner.username} получает {amount} ({desc})")

            # Устанавливаем current_win для ВСЕХ игроков (победители и проигравшие)
            for player in game.players:
                player_win = 0
                for w, amt, _ in winners:
                    if w.user_id == player.user_id:
                        player_win = amt
                        break
                player.current_win = player_win

            # Изменяем статус на FINISHED
            game.status = GameStatus.FINISHED

        # Обновляем статистику (отдельный флаг)
        if not game.stats_updated:
            game.stats_updated = True
            logger.info(f"Игра {game.game_id}: обновление статистики после showdown")

            # Победителями считаем тех, у кого НЕ "Возврат ставки"
            real_winners = [(w, amt, desc) for w, amt, desc in winners if desc != "Возврат ставки"]

            for winner, amount, desc in real_winners:
            ...
            for player in game.players:
                ...
                # Проигравшие - те кто не получил ничего или только возврат
                player_is_loser = True
                for w, amt, desc in winners:
                    if w.user_id == player.user_id and desc != "Возврат ставки":
                        player_is_loser = False
                        break

                if player_is_loser and player.total_bet > 0:
...
@router.callback_query(F.data.startswith("return_to_lobby:"))
async def cb_return_to_lobby(callback: CallbackQuery) -> None:
    """
    Вернуться в лобби с экрана результатов

    Переводит всех игроков в чистое лобби без игровых данных прошлой игры.
    """
    action, game_id, params = parse_callback_data(callback.data)
    chat_id = callback.message.chat.id
    ...
    # Фильтруем игроков с достаточным количеством фишек
    players_with_chips = [p for p in game.players if p.balance >= game.blinds_big]

    if not players_with_chips:
        await callback.answer("⚠️ Ни у кого нет фишек для продолжения!", show_alert=True)
        # Закрываем стол если нет игроков с фишками
        message_ids = await db.get_game_messages(game_id)
        for msg_id in message_ids:
            try:
                await callback.bot.delete_message(chat_id=chat_id, message_id=msg_id)
        ...
        await callback.bot.send_message(chat_id, "🚪 Стол закрыт - ни у кого не осталось фишек.")
        return

    # Обновляем список игроков (убираем тех, у кого недостаточно фишек)
    game.players = players_with_chips

    # Сбрасываем игру в состояние лобби
    game.reset_to_lobby()
    await save_game(game)
    ...
    await callback.message.edit_text(msg_text, reply_markup=keyboard)
    await callback.answer("🏠 Возвращаемся в лобби")
...
async def cb_refresh(callback: CallbackQuery) -> None:
    ...
    if game.status == GameStatus.WAITING:
        msg_text = format_waiting_message(game, game.players, None)
        keyboard = get_waiting_keyboard(game_id, len(game.players))
    elif game.status in [GameStatus.SHOWDOWN, GameStatus.FINISHED] and game.pot_distributed:
        # Показываем результаты showdown (из кэша, не модифицируя балансы)
        winners = game.determine_winners()
        msg_text = format_showdown_message(game, game.players, winners)
        keyboard = get_showdown_keyboard(game.game_id)
    else:
