...
async def cmd_balance(message: Message) -> None:
    ...
    /balance @username - показать баланс другого игрока
    """
    ...
    if len(command_parts) > 1:
        # Пытаемся найти упоминание пользователя
        if message.entities:
            for entity in message.entities:
                if entity.type == "mention":
                    # Извлекаем username из текста (убираем @)
                    mention_text = message.text[entity.offset:entity.offset + entity.length]
                    target_username = mention_text.lstrip('@')
                    break
        else:
            # Попытка парсить username вручную
            potential_username = command_parts[1].lstrip('@')
    ...
    if target_username:
        ...
        if not target_user:
            await message.answer(f"⚠️ Пользователь @{target_username} не найден в базе данных.")
            return
