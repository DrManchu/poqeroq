...
if __name__ == "__main__":
    ...
    print(f"Карт в колоде: {deck.remaining()}")
    
    # Раздача карт
    hand1 = deck.deal(2)
    hand2 = deck.deal(2)
    print(f"Рука 1: {format_cards(hand1)}")
    print(f"Рука 2: {format_cards(hand2)}")
    
    # Общие карты
    deck.burn()  # Сжигаем
    flop = deck.deal(3)
    print(f"Флоп: {format_cards(flop)}")
    
    deck.burn()
    turn = deck.deal(1)
    print(f"Тёрн: {format_cards(turn)}")
    
    deck.burn()
    river = deck.deal(1)
    print(f"Ривер: {format_cards(river)}")
    
    community = flop + turn + river
    print(f"\nСтол: {format_community_cards(community)}")
    print(f"Осталось карт: {deck.remaining()}")