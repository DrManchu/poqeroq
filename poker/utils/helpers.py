...
def format_username(username: Optional[str], user_id: int) -> str:
    """Форматировать имя пользователя для отображения (legacy, используйте get_mention)"""
    if username:
        return f"@{escape_html(username)}"
    return f"Игрок_{user_id}"


def get_display_name(first_name: Optional[str], username: Optional[str]) -> str:
    """
    Получить отображаемое имя пользователя для интерфейса

    Правило: использовать first_name, если нет - username, если нет - "Игрок"
    """
    if first_name:
...
def get_mention(first_name: Optional[str], username: Optional[str], user_id: int = None) -> str:
    """
    Получить упоминание пользователя для пингов/уведомлений

    Правило:
    - Если есть username - использовать @username
    - Иначе использовать first_name
    - Иначе "Игрок_{user_id}" или просто "Игрок"
    """
    if username:
        return f"@{escape_html(username)}"
    if first_name:
